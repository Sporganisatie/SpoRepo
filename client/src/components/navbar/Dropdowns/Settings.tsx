...
class SettingsDropdown extends Component<SettingsDropdownProps, SettingsDropdownState> {
  ...
  render() {
    return (
      <div
        className="hidden md:flex group h-full w-10 items-center"
        onPointerEnter={this.showMenu}
        onPointerLeave={this.closeMenu}
        onClick={this.showMenu}
      >
        <FontAwesomeIcon
          className="m-auto duration-500 transform group-hover:text-white group-hover:rotate-45"
          icon={faCog}
        />
        {this.state.showMenu ? (
          <div className="navbar_dropdown-content right-2">
            <Link className="navbar_settings_dropdown_item" to="/settings">
              <span>Settings</span>
            </Link>
            <button className="navbar_settings_dropdown_item" onClick={() => this.logout()}>
              Logout
            </button>
          </div>
        ) : null}
