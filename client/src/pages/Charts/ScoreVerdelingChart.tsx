...
const ScoreVerdelingChart = () => {
  ...
  useEffect(() => {
    axios
      .get(`/api/Charts/scoreSpread`, { params: { raceId, budgetParticipation } })
      .then((res) => {
        const usernames = res.data[0].usernamesAndScores.map(
  ...
  return (
    <div className="chart-frame">
      <BarChart width={chartdata.data.length * 70} height={600} data={chartdata.data}>
        <CartesianGrid
          strokeDasharray="3 3"
          verticalPoints={CalcVerticalLines()}
          vertical={false}
        />
        <XAxis dataKey="name">
        ...
        <YAxis tickCount={10}>
          <Label value="Score" angle={-90} position="left" offset={-10} />
        </YAxis>
        <Tooltip
        ...
        {chartdata.usernames.map((username, index) => (
          <Bar key={index} dataKey={username} fill={colors[index]} isAnimationActive={false} />
        ))}
      </BarChart>
    </div>
  );
