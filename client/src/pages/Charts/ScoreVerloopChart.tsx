...
const ScoreVerloopChart = () => {
  ...
  const [filters, setFilters] = useState<{
    toggles: { username: string; showUser: boolean }[];
    center: string;
  ...
  useEffect(() => {
    const url = positieVerloop
      ...
      : perfectPoints
        ? "perfectScoreVerloop"
        : "scoreVerloop";
    axios.get(`/api/Charts/${url}`, { params: { raceId, budgetParticipation } }).then((res) => {
  ...
  const updateFiltered = useCallback((): void => {
  ...
  return (
    <div style={{ backgroundColor: "#222", padding: "20px" }}>
      ...
      <div>
        <LineChart width={Math.min(data.data.length * 70, 1540)} height={600} data={filteredData}>
          <CartesianGrid vertical={false} strokeDasharray="3 3" />
          <XAxis dataKey="Name">
            <Label value="Etappe" position="bottom" dy={-15} />
          </XAxis>
          {positieVerloop ? (
            <YAxis tickCount={10} tickFormatter={(value) => (-value).toString()}>
          ...
          {data.usernames
            ...
            .filter((_, index) => filters.toggles.at(index)?.showUser)
            .map((user) => (
              <Line
                key={user.index}
                type="linear"
