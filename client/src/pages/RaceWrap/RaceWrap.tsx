...
const RaceWrap = () => {
  ...
  useEffect(() => {
    axios.get(`/api/race/all`).then((res) => {
  ...
  useEffect(() => {
    ...
    setTotals(totals);
    const percentages = totals?.map((t) => Math.round((t.gemist / t.optimaal) * 1000) / 10) ?? [];
  ...
  useEffect(() => {
    const lostBudgets = uitvallers?.map((u) => u.uitvallerStagesBudget) ?? [];
  ...
  return (
    <div className="center-stack" style={{ marginBottom: "4rem" }}>
      ...
      {pietjePrecies ? (
        <RaceWrapAward awardName={"🎯 Beste dagopstellingen"} awardWinners={pietjePrecies} />
      ) : (
        <></>
      )}
      {missedPoints ? <RaceWrapMissedPoints missedPoints={totals} /> : <></>}

      {pechvogel ? (
        <RaceWrapAward awardName={"🦆 Pechvogel Prijs"} awardWinners={pechvogel} />
      ) : (
        <></>
      )}
      <Uitvallers />
      <RaceWrapStages />
      <RaceWrapAward awardName={"Vergelijking met andere races"} awardWinners={""} />
      {raceScoreData ? <RaceWrapRaceScores raceScoreData={raceScoreData} raceId={raceId} /> : <></>}
    </div>
  );
};
...
function getMissedPointsTotal(missedPoints?: MissedPointsTableData[]) {
  ...
  const pointTotals = missedPoints
    .map((userData) => {
      const data = userData.data.find(({ etappe }) => etappe === "Totaal");
      if (!data) {
  ...
  const optimalOrder = pointTotals.slice().sort((a: any, b: any) => b.optimaal - a.optimaal);
