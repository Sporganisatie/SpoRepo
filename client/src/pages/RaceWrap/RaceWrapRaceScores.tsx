...
const RaceWrapRaceScores = ({
  ...
  const [greatestScore, setGreatestScoreRanking] = useState<(RaceScore & { diff: number })[]>([]);
  ...
  useEffect(() => {
    setTopscoreRanking(
      raceScoreData
        ...
        .filter(
          (data) => data.rank < 6 || data.rank > raceScoreData.length - 5 || data.raceId === raceId
        )
    );
      ...
      raceScoreData
        ...
        .map((data, idx) => {
          return {
            ...data,
            rank: idx + 1,
            username: data.usernamesAndScores[data.usernamesAndScores.length - 1].username,
          };
        })
        .filter(
          (data) => data.rank < 6 || data.rank > raceScoreData.length - 5 || data.raceId === raceId
