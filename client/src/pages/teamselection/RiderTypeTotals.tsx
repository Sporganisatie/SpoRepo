...
function sumType(team: RiderParticipation[], typeName: string): string {
    return team.filter(r => r.type === typeName).reduce((sum, current) => sum + current.price / 1000000, 0) + 'M';
}
...
const RiderTypeTotals = ({ team }: { team: RiderParticipation[] }) => {
    var totals: Array<RiderTypeTotalRow> =
        [
            {
                rowName: "Budget", klassement: sumType(team, "Klassement"),
            ...
            {
                rowName: "Aantal", klassement: countType(team, "Klassement"),
